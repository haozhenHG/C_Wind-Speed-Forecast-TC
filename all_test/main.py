...
datatype = 'NP'
filename = './datasets/{}.csv'.format(datatype)  # filename = NP.CSV
vmdsavename = './result/VMD_{}.csv'.format(datatype)
savename = './result/VMD_SE_{}.csv'.format(datatype)
f = pd.read_csv(filename,usecols=[3]) # 默认选取第 4 列（索引为 3）作为分解样本
...
# 中心模态
fi = pd.read_csv(filename,index_col=0)   # filename = NP.CSV
colorchose = {0:'coral',1:'orange',2:'steelblue'}
for i in range(3):
    a = fi.values[:,i]   # 数据集的每一列
...
if(os.path.exists(savename)):
    ...
    f = pd.read_csv(vmdsavename,index_col=0) #  # vmdsavename = './result/VMD_{}.csv'.format(datatype)
    print(f.values.shape)
    reset=0
    se_set = {}
    se_list =[]
    for i in range(0,f.values.shape[1]-1):  # 使用for循环遍历数据的每一列（模态）
        sampen_of_series = sampen2(f.values[:10000, i])
        se_set.update({i:sampen_of_series[2][1]})
        se_list.append(sampen_of_series[2][1])
    print(se_set)
    print(se_list)
    se_list.sort(reverse=True)   # 排序
    print(se_list)
    ...
    for i in range(0,f.values.shape[1]-1):
        if se_set[i] in se_list[0:2]:    # 人为规定这是高
            high += f.values[:,i]
        elif se_set[i] in se_list[2:6]:  # 这是中
            middle += f.values[:, i]
        elif se_set[i] in se_list[6:8]:  #  这是低
            low += f.values[:, i]
    out_put[:,0] = high
    out_put[:,1] = middle
    out_put[:,2] = low
    ...
    for i in range(3):
        ...
        pdflie = pd.concat([pdflie,dataframe],axis=1)
    for i in range(1,4):
        f = pd.read_csv(filename,usecols=[i]) # datatype = 'NP'  filename = './datasets/{}.csv'.format(datatype)
